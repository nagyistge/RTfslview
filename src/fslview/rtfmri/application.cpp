...
{ 
  ...
  try {
      ...
      if ( info->isStatImage() || info->isMaskImage() )
	{
      ...
      if(!lut) lut = LookUpTable::redYellow();
...
bool ApplicationWindow::loadFile(const QString & absFilePath,QString lookuptable)
{ 
  bool status(true);
  ...
  try {
      ...
      if ( info->isStatImage() || info->isMaskImage() )
	{
      ...
      if((!lut) && (lookuptable != "")) lut = LookUpTable::greyScale();
...
void ApplicationWindow::fileOpen(QString fn, QString lookuptable)
{
  
  if(!fn.isEmpty()) {
    loadFile(fn,lookuptable);
    if(ModelFit::isFeatDir(fn))
      loadFeat(fn);
    setCurrentDir(fn);
...
void ApplicationWindow::fileAdd(QString fn,QString lookuptable)
{
  
  if(!fn.isNull())setCurrentDir(fn);

  if(!fn.isEmpty()) {
    loadOverlay(fn,lookuptable);
...
void ApplicationWindow::RTLoop(QString fn)
{
  ...
  string fbase = fn.data();
  QString RTinputdir = QString(fbase.substr(0,fbase.rfind("/")));
  ...
  while( errcnt < 5)
  {
	try //Since data is transferred asynchronously, sometimes we try to read the file before it is completely written
	{
		...
		else
		{
			...
			fileAdd(fn,"redyellow");
			//viewImageHistogram(true);
			break;
		}  
  ...
  QString RToutputdir = rtthread->get_outputdir();
  rename(RTinputdir+"/"+fbase+".dcm",RToutputdir+fbase+".dcm");	   
  //rename(file.data(),outfile.data() );
  rtthread->set_fileread();
}

void ApplicationWindow::runRTExp(QString inputdir, QString mask,int NRT,int buffer)
{
...
void ApplicationWindow::viewrtfMRIDialog()
{
  rtfMRIDialog* rtfmriD = new rtfMRIDialog(this, "rtfMRI Settings", false,WStyle_Customize | WStyle_NormalBorder|WStyle_Title);
  int test = rtfmriD->exec();
  if (test == 1 )
	{
		rtfMRIAction->setEnabled(false);
		EndRTAction->setEnabled(true);
		runRTExp(rtfmriD->inputdir, rtfmriD->mask,rtfmriD->NTR,rtfmriD->BS);
...
bool ApplicationWindow::loadOverlay(const QString & absFilePath,QString lookuptable)
{
  bool status(true);
  ...
  try{
	...
	qApp->processEvents();
	    ...
	    //lut = m_imageGroup->getLut(info->inqLutName());
            ...
            //if((!lut) && (lookuptable != "")) lut = LookUpTable::redYellow();
	    if (lookuptable != "")	    
		m_imageGroup->addLookUpTable(LookUpTable::greyScale());
	    m_cursor->setVMax(overlay->getInfo()->inqNumVolumes());
	    ...
	    new HistogramWidget(m_ws, overlay->getVolume(v),
				...
				v, overlay->getInfo()->isInteger(),true);   
...
void ApplicationWindow::viewImageHistogram(bool redraw)
{
  if(m_imageGroup.use_count())
  {  
    if(ViewWidget *view = dynamic_cast<ViewWidget*>(m_ws->activeWindow())) {
	...
	if(Image::Handle image = ol->inqActiveImage()) {
	  unsigned int v = m_cursor->inqV();
	  //histogram =  HistogramWidget(m_ws, image->getVolume(v),
	  //			image->getInfo()->inqImageName(),
		//		v, image->getInfo()->isInteger(), redraw);  
          connect( histogram, SIGNAL(windowClose(QCloseEvent*)),
                   this, SLOT(childWindowClose(QCloseEvent*))); 
          viewShow(histogram);
        }
